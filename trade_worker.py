...
class TradeWorker():
    ...
    async def send_trade(self, trade):
        if self.double_check:
            ...
            while True:
                try:
                    ...
                    logging.error(f"Connect/Read timed out while trying to grab trade status info for double-check: {self.trade_type}: {traceback.format_exc()}")
                    print_timestamp(f"Connect/Read timed out while trying to grab trade status info for double-check: {self.trade_type}")
        ...
        try:
            ...
            logger.critical(f"Unknown error while grabbing rolimons data: {traceback.format_exc()}\nRolimons data is now: {self.roli_data}")
        ...
        try:
            ...
            logging.error(f"Unable to send {self.trade_type} trade webhook: {trade['id']} {traceback.format_exc()}")
