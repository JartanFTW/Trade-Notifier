...
class TradeWorker():
    ...
    async def create(cls, main_folder_path: str, user: User, webhook_url: str, update_interval: int, theme_name: str, trade_type: str = "Completed", add_unvalued_to_value: bool = True, testing: bool = False, double_check: bool = False, webhook_content: str = ""):
        ...
        if testing:
            ...
            try:
                ...
                logger.warning(f"No {self.trade_type} trades in history to send test webhook based on")
                print_timestamp(f"No {self.trade_type} trades to send a test webhook based on, skipping")
        return self
    ...
    async def send_trade(self, trade):
        if self.double_check:
            ...
            while True:
                try:
                    ...
                    logger.warning(f"Timed out while trying to grab {self.trade_type} trade status info: {traceback.format_exc()}")
                    print_timestamp(f"Timed out while trying to grab {self.trade_type} trade status info")
                    await asyncio.sleep(5)
            if trade['id'] not in [trade['id'] for trade in trades_info['data'][::-1]]:
                print_timestamp(f"{self.trade_type} trade {trade['id']} detected as fake, skipping notification")
                return
        
        try:
            self.roli_data = await get_roli_data()
        except httpx.ReadTimeout:
            logger.error(f"Timed out while trying to grab roli data: {traceback.format_exc()}")
            print_timestamp("Timed out while trying to grab roli data")
        except Exception:
            logger.error(f"Unknown error while grabbing rolimons data: {traceback.format_exc()}")
        ...
        try:
            ...
            logger.error(f"Unable to send {self.trade_type} trade webhook: {trade['id']} {traceback.format_exc()}")
            print_timestamp(f"Unable to send {self.trade_type} trade webhook: {trade['id']}")
